...
public:
	...
	void imprimeBFS() const;	//busca em largura
	void imprimeDFS_pre() const;	//busca em profundidade
...
private:
	...
	Node<T> * igualtree( Node<T>*, Node<T>* ) const;
...
MySet<T> & MySet<T>::operator=( const MySet<T> &other ){
	...
	root = igualtree(other.root, NULL);

	return *this;
}

template <class T>
Node<T> * MySet<T>::igualtree( Node<T> *curr, Node<T> *parent ) const{
	if( !curr ) return curr;

	Node<T> *NewRoot = new Node<T>(curr->elem);

	NewRoot->left = igualtree( curr->left, curr );
	NewRoot->right = igualtree( curr->right, curr );
